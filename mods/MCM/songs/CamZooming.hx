...
public var camZoomMult:Float = 1; //i love being lazy (for change stage zoom specifically)

public var forceDefaultCamZoom:Bool = false; //for songs that already have bf/dad cam zoom values

public var gameLerpAmt:Float = 0.05;
...
function update(elapsed:Bool) {
    ...
    if (lerpCam) {
        ...
        FlxG.camera.zoom = lerp(FlxG.camera.zoom, stageZoom == -1 ? defaultCamZoom : stageZoom, gameLerpAmt);
    }
}
